package com.zenyone.verify.util;
...
public class Base64Util {
	...
	/**
	 * 字节数组编码到字符数组
	 * @param sArr byte[] 字节数组
	 * @param lineSep boolean 是否添加行分隔符(为true时每隔76个字符添加行分隔符"\r\n")
	 ...
	 * 字节数组编码到字节数组
	 * @param sArr byte[] 字节数组
	 * @param lineSep boolean 是否添加行分隔符(为true时每隔76个字节添加行分隔符"\r\n")
	...
	public final static byte[] decodeFast(String str) {
		...
		// get the padding count (=) (0, 1 or 2)
		int pad = str.charAt(eIx) == '=' ? (str.charAt(eIx - 1) == '=' ? 2 : 1) : 0; // Count
		// '='
		// at
		// end.
		int cCnt = eIx - sIx + 1; // Content count including possible
		// separators
		int sepCnt = sLen > 76 ? (str.charAt(76) == '\r' ? cCnt / 78 : 0) << 1
				: 0;
		...
		for (int cc = 0, eLen = (len / 3) * 3; d < eLen;) {
			// Assemble three bytes into an int from four "valid" characters.
			int i = IA[str.charAt(sIx++)] << 18 | IA[str.charAt(sIx++)] << 12
					| IA[str.charAt(sIx++)] << 6 | IA[str.charAt(sIx++)];
	...
	public static String decode(String str) {
		try {
			return new String(Base64Util.decodeFast(str), "UTF-8");
		} catch (UnsupportedEncodingException e) {
			System.err.println("Base64 encoding error: " + e.getMessage());
			e.printStackTrace();
		}
